...
import (
	...
	"github.com/abhinandpn/go-grpc-order-svc/pkg/models"
...
func (s *Server) CreateOrder(ctx context.Context, req *pb.CreateOrderRequest) (*pb.CreateOrderResponse, error) {
	product, err := s.ProductSvc.FindOne(int(req.ProductId))
	if err != nil {
		...
		return &pb.CreateOrderResponse{Status: http.StatusConflict, Error: "stock is too less"}, nil
	}
	order := models.Order{
	...
	if err != nil {
		return &pb.CreateOrderResponse{
			Status: http.StatusBadRequest, Error: err.Error()}, nil
